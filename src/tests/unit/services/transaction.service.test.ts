...
import {
  ...
  findTransaction,
} from '../../../services/database/transaction.service';
...
describe('Update or create Transaction', () => {
  ...
  test('should create a new transaction', async () => {
    ...
    // We expect to get only 1 transaction in db
    expect(newTransaction.vtexPaymentId).toEqual(transactionMock.vtexPaymentId);
  });
  test('should find an existing transaction', async () => {
    // Create transaction
    await createTransaction(transactionMock);
    // find transaction
    const transaction = await findTransaction({ vtexPaymentId: transactionMock.vtexPaymentId });
